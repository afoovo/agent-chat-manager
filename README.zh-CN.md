...
- **数据仪表盘** — 使用统计、成本追踪、Token 用量、模型/Agent 分布图表
- **导出导入** — 导出会话为 `.db` 文件，支持从其他机器导入数据
- **书签标注** — 为重要会话添加书签和自定义标签
- **暗色模式** — 明暗主题一键切换，自动跟随系统设置
...
1. 下载项目
2. 双击 `首次安装.bat`（`setup.bat`）安装依赖（仅首次）
3. 双击 `一键启动.bat`（`start.bat`）启动 — 浏览器自动打开 http://localhost:5173
...
cd frontend
npm install
npm run dev:all                # 单窗口同时启动前后端（带颜色前缀区分）
```
...
│   ├── db.py                  # SQLite 查询层（只读模式）
│   ├── db_search.py           # FTS5 全文搜索引擎 + 缓存
│   ├── db_import.py           # 多机数据导入管理
│   └── routes/                # API 接口
├── frontend/
│   ├── src/
│   │   ├── components/        # React 组件（侧边栏、会话列表、仪表盘等）
│   │   ├── hooks/             # 自定义 hooks (useTheme, useBookmarks)
│   │   └── lib/               # API 客户端、类型定义、工具函数
│   ├── package.json
│   └── vite.config.ts         # Vite 开发服务器（端口 5173，/api 代理到后端）
├── 首次安装.bat / setup.bat     # 一键环境安装
└── 一键启动.bat / start.bat     # 一键启动服务
```
